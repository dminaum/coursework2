...
def from_api_to_list(api_vacancies):
    ...
    for api_vacancy in api_vacancies:
        ...
        requirement = api_vacancy.get("snippet", {}).get(
            "requirement", "Нет информации"
        )
...
def filter_vacancies_by_salary(
    vacancies: list[Vacancy], salary_from, salary_to
) -> list[Vacancy]:
    """
...
def get_top_vacancies(vacancies, top_n):
    ...
    sorted_vacancies = sorted(
        vacancies, key=lambda vacancy: vacancy.salary_from, reverse=True
    )
...
def filter_vacancies_by_keyword(vacancies, keywords):
    ...
    return [
        vacancy
        for vacancy in vacancies
        if any(keyword.lower() in vacancy.requirement.lower() for keyword in keywords)
    ]
